...
DEFAULT_OUTPUT_DIR = Path(
    "outputs/masked_transformer_mixed_mask_v5"
)
...
def load_initial_transformer(
    ...
    requested_config = dict(model.get_config())
    requested_config.pop(
        "use_aligned_respiration_conditioning",
        None,
    )

    if checkpoint_model_config != requested_config:
        raise ValueError(
...
def main():
    ...
    model = RespirationToEEGTransformer(
        ...
        # V5 deliberately keeps the original globally
        # concatenated conditioning path. The explicit
        # time-aligned path is reserved for V6.
    ...
    print("Device:", device)
    print(
        "Model: codebook-tied mixed-mask Transformer V5"
    )
    ...
    for epoch_index in range(args.epochs):
        ...
        checkpoint = {
            ...
            # Preserve the exact legacy V3/V5 checkpoint
            # schema expected by the V6 warm-start loader.
